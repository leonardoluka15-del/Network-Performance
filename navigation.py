...
def get_auto_refresh_seconds():
    interval = st.session_state.get("auto_refresh_interval", "Off")
    refresh_mapping = {
        "Off":
            None,
        "30 seconds":
    ...
    return refresh_mapping.get(interval)

# GLOBAL CSS
def load_css(current_page="Overview"):
    initialize_dashboard_settings()
    # CURRENT THEME
    theme = st.session_state.get("dashboard_theme", "Light")
    # THEME COLORS
    if theme == "Dark":
    ...
    page_key_map = {
        ...
        "KPI Upload": "nav_upload",
        "Site Analytics": "nav_site",
        "Report": "nav_report",
        "Alarms": "nav_alarms",
        "Settings": "nav_settings"
    }
    if current_page in page_key_map:
        active_key = page_key_map[current_page]
        active_css += f"""
        .st-key-{active_key} button,
        .st-key-{active_key} button p,
        .st-key-{active_key} button span {{color: #ff6258 !important;}}
        """
    # ACTIVE TECHNOLOGY
    if current_page in ["2G", "3G", "4G", "5G", "VoLTE"]:
        active_css += """
        .st-key-tech_nav
...
.st-key-network_header {{
    ...
    margin: 0 !important;
    border-bottom: 6px solid #ff6258 !important;
...
.stApp .dashboard-title {{color: {text_primary} !important;}}
...
.stApp .dashboard-subtitle {{color: {text_secondary} !important;}}
...
.stApp .panel-title,
.stApp .section-title {{color: {text_primary} !important;}}
.stApp .panel-subtitle {{color: {muted_text} !important;}}
...
.stApp .kpi-label {{color: {text_secondary} !important;}}
...
.stApp .technology-kpi-value {{color: {text_primary} !important;}}
...
.stApp .settings-current {{color: {text_secondary} !important;}}
...
.stApp .kpi-delta-positive {{color: #22c55e !important;}}
.stApp .kpi-delta-negative {{color: #ff6258 !important;}}
...
[data-testid="stDataFrame"] {{
    ...
    background-color: {dataframe_background} !important;
    border-color: {dataframe_border} !important;
}}
...
# NAVIGATION
# ============================================================
def navigation(current_page="Overview"):
    initialize_dashboard_settings()
    # LOAD GLOBAL THEME
    load_css(current_page)
    # BLACK HEADER
    with st.container(key="network_header"):
        # HEADER COLUMNS
        (logo_col, overview_col, tech_col, upload_col, site_col, report_col, alarm_col, settings_col, login_col) =st.columns(
            [
                1.45,   # Logo
                0.95,   # Overview
                1.40,   # Technology
        ...
        with tech_col:
            technology_choice = st.selectbox("Technology", ["TECHNOLOGY", "2G", "3G", "4G", "5G", "VoLTE"], key="tech_nav", label_visibility="collapsed")
            if technology_choice == "2G": st.switch_page("pages/technology_2G.py")
            elif technology_choice == "3G": st.switch_page("pages/technology_3G.py")
            elif technology_choice == "4G": st.switch_page("pages/technology_4G.py")
            elif technology_choice == "5G": st.switch_page("pages/technology_5G.py")
            elif technology_choice == "VoLTE": st.switch_page("pages/technology_VoLTE.py")

        # KPI UPLOAD
        with upload_col:
            if st.button("KPI UPLOAD", key="nav_upload", use_container_width=True):
                st.switch_page("pages/3_KPI_Upload.py")
        # SITE ANALYTICS
        with site_col:
            if st.button("SITE ANALYTICS", key="nav_site", use_container_width=True):
                st.switch_page("pages/4_Site_Analytics.py")
        # REPORT
        with report_col:
        ...
        # ALARMS
        with alarm_col:
            if st.button("ALARMS", key="nav_alarms", use_container_width=True):
                st.switch_page("pages/6_Alarms.py")
        # SETTINGS
        with settings_col:
